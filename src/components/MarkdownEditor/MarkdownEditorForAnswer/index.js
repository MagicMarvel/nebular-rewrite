...
export default function Index(props) {
  ...
  const handlePhotoUpload = async (event) => {
    ...
    if (event.target.files.length !== 0) {
      ...
      setImageFileName(file.name.replace(/[\s]/g, "_") || "");

      // 拼接成form放入请求参数中
      let param = new FormData();
      param.append("file", file, file.name.replace(/[\s]/g, "_"));
  ...
  return (
    <div
      className="absolute z-40  min-h-screen text-gray-700 py-6"
      style={{ top: "-50vh", width: "95vw", left: "-6vw" }}
    >
