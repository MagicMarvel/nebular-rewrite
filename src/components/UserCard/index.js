...
export default function Index(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(getUsrPhoto(usrInformation.username), {
          responseType: "blob",
        });
          ...
          <div className="flex flex-col  items-center justify-between h-52 transition-all">
            ...
            >
              {photoBlob === undefined ? (
                ...
                <img src={photoBlob} className="h-20 w-20" alt="用户头像" />
              )}
            </div>
            <div className="border-l flex-grow border-blue-300"></div>
            <div className="transition-all duration-1000 h-0 overflow-hidden group-hover:h-16 ">
              <div className="text-gray-800">{usrInformation.nickname}</div>
